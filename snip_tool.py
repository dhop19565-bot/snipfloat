...
class SelectionOverlay:
    def __init__(self, on_done):
        ...
        self.root.attributes("-alpha", 0.01)   # nearly invisible throughout
        ...
        # Crosshair lines (full screen)
        sw = self.root.winfo_screenwidth()
        sh = self.root.winfo_screenheight()
        self.h_line = self.canvas.create_line(0, 0, sw, 0, fill="#7aa2f7", width=1, dash=(4, 4))
        self.v_line = self.canvas.create_line(0, 0, 0, sh, fill="#7aa2f7", width=1, dash=(4, 4))

        # Selection rectangle (drawn after drag starts)
        self.rect_outer = None   # white border (contrast)
        self.rect_inner = None   # blue dashed marching ants
        self.fill_rect  = None   # semi-transparent fill
        self.size_label = None   # WxH indicator
    ...
    # ── Crosshair follows cursor before drag ──────────────────────────────
    def on_mouse_move(self, e):
        if not self.dragging:
            self.canvas.coords(self.h_line, 0, e.y, self.root.winfo_screenwidth(), e.y)
            self.canvas.coords(self.v_line, e.x, 0, e.x, self.root.winfo_screenheight())
    ...
    def on_press(self, e):
        ...
        # Build selection shapes
        self.fill_rect  = self.canvas.create_rectangle(
            e.x, e.y, e.x, e.y, fill="#7aa2f7", stipple="gray25", outline="")
    ...
    def _update_rect(self):
        ...
        # Position label just outside bottom-right of selection
        lx = max(x1, x2) + 4
        ly = max(y1, y2) + 4
        sw = self.root.winfo_screenwidth()
        sh = self.root.winfo_screenheight()
